...
namespace WpfApplication1.Ui.DesignerWithPropreryGridDialog
{
    public class EditedViewModel : ViewModelBase, IDialogViewModel
    {
        ...
        private PropertyGrid.EditedViewModel _propertyGridViewModel;
        public PropertyGrid.EditedViewModel PropertyGridViewModel
        {
        ...
        public EditedViewModel(int? zoneId = null, Shp locationPoint = null)
        {
            ...
            PropertyGridViewModel = new PropertyGrid.EditedViewModel();
        ...
        private void OnShpReceived(Shp shp)
        {
            if (shp is PathShp)
            {
                PropertyGridViewModel = new PropertyGrid.Pipe.EditedViewModel(shp.Id);
            }
            else if (shp is EllipseShp)
            {
                PropertyGridViewModel = new PropertyGrid.Junction.EditedViewModel(shp.Id);
            }
            else if (shp is RectangleShp)
            {
                PropertyGridViewModel = new PropertyGrid.CustomerNode.EditedViewModel(shp.Id);
            }
